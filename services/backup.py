...
async def backup_bot_db(bot: "Bot") -> bool:
    ...
    try:
        ...
        try:
            ...
            # شمارش همه جداول — کامل
            counts: dict = {}
            all_tables = (
                "users", "subscriptions", "payments", "plans",
                "tickets", "ticket_messages", "discount_codes",
                "admin_settings", "referrals", "test_subscription_records",
            )
            ...
            row_summary = (
                f"👥 کاربران: <b>{counts.get('users',0)}</b>  "
                f"📦 اشتراک: <b>{counts.get('subscriptions',0)}</b>  "
                f"💰 پرداخت: <b>{counts.get('payments',0)}</b>\n"
                f"📋 پلن: <b>{counts.get('plans',0)}</b>  "
                f"🎫 تیکت: <b>{counts.get('tickets',0)}</b>  "
                f"💬 پیام تیکت: <b>{counts.get('ticket_messages',0)}</b>\n"
                f"🏷 تخفیف: <b>{counts.get('discount_codes',0)}</b>  "
                f"👥 رفرال: <b>{counts.get('referrals',0)}</b>  "
                f"🎁 تست: <b>{counts.get('test_subscription_records',0)}</b>\n"
                f"⚙️ تنظیمات: <b>{counts.get('admin_settings',0)}</b> مورد\n"
            )
        ...
        caption = (
            ...
            f"{row_summary}"
            f"\n{settings_summary}"
            f"✅ شامل: کاربران، اشتراک‌ها، پرداخت‌ها، پلن‌ها، تیکت‌ها،\n"
            f"   پیام‌های تیکت، تخفیف‌ها، رفرال‌ها، تنظیمات\n\n"
            f"♻️ <b>روش restore:</b>\n"
...
async def _backup_postgres(bot: "Bot", ts: str) -> bool:
    ...
    try:
        ...
        tables = [
            "users", "subscriptions", "payments", "tickets",
