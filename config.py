...
class Settings(BaseSettings):
    ...
    def parse_admin_ids(cls, v: object) -> List[int]:
        ...
        همه فرمت‌های رایج پشتیبانی می‌شوند:
          ...
          [5797901827,9876543210]
          ["5797901827","9876543210"]
          5797901827 9876543210   (space-separated)

        بهبودها:
          - فاصله بین آیدی‌ها (space) هم پشتیبانی می‌شود
          - کاراکترهای Unicode زائد (zero-width space و ...) حذف می‌شوند
          - لاگ debug برای عیب‌یابی
        """
        import re as _re
        if isinstance(v, list):
            result = [int(x) for x in v if str(x).strip()]
            return result
        if not isinstance(v, str):
            return []
        # پاکسازی کاراکترهای Unicode زائد (Zero-Width Space, BOM, ...)
        v = v.strip()
        v = _re.sub(r'[\u200b\u200c\u200d\ufeff\xa0]', '', v)
        v = v.strip()
        if not v:
        ...
        if v.startswith("["):
            ...
            try:
                ...
                v = v.strip("[]")
        # جایگزینی space با ویرگول (space-separated) و پاکسازی ویرگول‌های اضافه
        v = _re.sub(r'[\s,]+', ',', v)
        # فرمت ویرگول‌جدا: 123,456,789
        result = [int(x.strip()) for x in v.split(",") if x.strip().lstrip("-+").isdigit()]
        return result
...
def get_settings() -> Settings:
    ...
    # ── Fallback مستقیم از os.environ ─────────────────────────────────
    # اگه pydantic-settings نتوانست ADMIN_IDS را parse کند،
    # مستقیم از os.environ می‌خوانیم — این تضمین می‌کند Docker env_file
    # همیشه کار کند حتی اگه validator مشکل داشت.
    if not s.admin_ids:
        import os, re as _re
        raw = os.environ.get("ADMIN_IDS", "").strip()
        if raw:
            raw = _re.sub(r'[\u200b\u200c\u200d\ufeff\xa0]', '', raw)
            raw = _re.sub(r'[\s,]+', ',', raw)
            fallback_ids = [
                int(x.strip())
                for x in raw.split(",")
            ...
            if fallback_ids:
                # pydantic model رو نمی‌توانیم مستقیم تغییر بدیم
                # یه subclass موقت می‌سازیم با admin_ids پر شده
                object.__setattr__(s, "admin_ids", fallback_ids)
