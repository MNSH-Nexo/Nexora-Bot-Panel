...
from middlewares.rate_limit import RateLimitMiddleware, BanCheckMiddleware
...
async def main() -> None:
    ...
    # ── لاگ admin_ids برای عیب‌یابی ──────────
    import os as _os
    _raw_env = _os.environ.get("ADMIN_IDS", "(تنظیم نشده)")
    if settings.admin_ids:
        logger.success(f"✅ ADMIN_IDS بارگذاری شد: {settings.admin_ids}  (raw env: {_raw_env!r})")
    else:
        logger.error(
            f"❌ ADMIN_IDS خالی است!\n"
            f"  مقدار raw در محیط: {_raw_env!r}\n"
            f"  مطمئن شوید .env شامل: ADMIN_IDS=5797901827\n"
            f"  سپس: docker compose down && docker compose up -d"
        )
    ...
    # ── Middleware‌ها ─────────────────────────
    # Ban check: کاربران بن‌شده را در همه پیام‌ها و callback‌ها بلاک می‌کند
    _ban_mw = BanCheckMiddleware(admin_ids=list(settings.admin_ids))
    dp.message.middleware(_ban_mw)
    dp.callback_query.middleware(_ban_mw)
