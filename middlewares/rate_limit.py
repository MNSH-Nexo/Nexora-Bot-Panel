...
class BanCheckMiddleware(BaseMiddleware):
    """
    جلوگیری از استفاده کاربران بن‌شده.
    ادمین‌ها از این چک معاف‌اند.
    """

    def __init__(self, admin_ids: Optional[list] = None) -> None:
        self._admin_ids: list = admin_ids or []
    ...
    async def __call__(
        ...
        try:
            from database import AsyncSessionLocal
            from database.crud import get_user_by_telegram_id as _get_user
            async with AsyncSessionLocal() as session:
                db_user = await _get_user(session, uid)
