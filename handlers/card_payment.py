...
async def _notify_admins_receipt(
    ...
    """ارسال رسید به همه ادمین‌ها — از admin_notify مرکزی."""
    from services.admin_notify import notify_card_payment
    tg_user = receipt_msg.from_user
    await notify_card_payment(
        bot=bot,
        order_id=order_id,
        plan_name=plan_name,
        ...
        final_price_usdt=final_price_usdt,
...
async def card_receive_receipt(message: Message, state: FSMContext) -> None:
    """دریافت رسید — فقط عکس قبول می‌شود."""
    data = await state.get_data()
    ...
    # فقط عکس قبول است — متن، استیکر، فایل و ... رد می‌شوند
    if not message.photo:
        await message.answer(
            "🖼 <b>لطفاً عکس رسید پرداخت را ارسال کنید.</b>\n\n"
            "⚠️ فقط تصویر رسید پذیرفته می‌شود.\n"
            "متن، فایل یا سایر پیام‌ها قابل قبول نیستند.\n\n"
            "📌 برای لغو پرداخت /cancel را بزنید.",
            parse_mode="HTML",
        )
        return
