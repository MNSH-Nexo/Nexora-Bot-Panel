...
async def create_ticket(
    ...
    msg = TicketMessage(
        ...
        sender_id=sender_id,
        body=first_message,
        is_admin_reply=initiated_by_admin,
    )
    session.add(msg)
    await session.commit()
    await session.refresh(ticket)
    logger.info(f"تیکت جدید #{ticket.id} از user_id={user_id} initiated_by_admin={initiated_by_admin}")
    return ticket
...
async def create_ticket_by_admin(
    ...
    """
    ایجاد تیکت از طرف ادمین برای یک کاربر خاص.
    - user_id = کاربر هدف (صاحب تیکت)
    - sender_id = آیدی DB ادمین
    - initiated_by_admin = True
    - اولین پیام is_admin_reply=True
    """
...
async def delete_ticket(session: AsyncSession, ticket_id: int) -> bool:
    """
    حذف کامل یک تیکت و تمام پیام‌هایش.
    Returns: True اگر پیدا و حذف شد.
    """
    from sqlalchemy import delete as sa_delete
    ticket = await get_ticket(session, ticket_id)
    if not ticket:
        return False
    # پیام‌ها cascade delete دارند — فقط ticket رو حذف کن
    await session.execute(sa_delete(Ticket).where(Ticket.id == ticket_id))
    await session.commit()
    logger.info(f"تیکت #{ticket_id} حذف شد.")
    return True
...
_ADMIN_LOCK_KEY = "admin_login_locked"


async def get_all_db_admins(session: AsyncSession) -> List[User]:
    """همه کاربرانی که is_admin=True دارند (از DB — بدون .env)."""
    result = await session.execute(
        select(User).where(User.is_admin == True).order_by(User.telegram_id)
    )
    return list(result.scalars().all())


async def revoke_admin(session: AsyncSession, telegram_id: int) -> bool:
    """
    دسترسی ادمین کاربر را لغو می‌کند.
    Returns: True اگر کاربر پیدا و آپدیت شد.
    """
    result = await session.execute(
        select(User).where(User.telegram_id == telegram_id)
    )
...
async def is_admin_login_locked(session: AsyncSession) -> bool:
    """آیا قفل ورود ادمین فعال است؟"""
    val = await get_setting(session, _ADMIN_LOCK_KEY, "0")
    return val == "1"


async def set_admin_login_lock(session: AsyncSession, locked: bool) -> None:
    """قفل ورود ادمین را فعال یا غیرفعال می‌کند."""
    await set_setting(session, _ADMIN_LOCK_KEY, "1" if locked else "0")
...
async def ban_user(session: AsyncSession, telegram_id: int) -> bool:
    """بن کردن کاربر — Returns True اگر پیدا و آپدیت شد."""
...
async def unban_user(session: AsyncSession, telegram_id: int) -> bool:
    """رفع بن کاربر — Returns True اگر پیدا و آپدیت شد."""
    result = await session.execute(select(User).where(User.telegram_id == telegram_id))
    user = result.scalar_one_or_none()
    if not user:
        return False
